...
conversion_rates = {'fake_currency': {'rate': 1.3,
                                      'updated': datetime.now()}}  # init and show contents
...
def format_pricing(raw_pricing: dict):
    return {product["product_id"]: {"price": product["price"],
                                    "VAT": round(raw_pricing["vat_bands"][product["vat_band"]]*product["price"], 2)
...
def calc_order_details(order_quantities: dict, pricing: dict, currency : str, conversion_rate: float = 1.0):
    summary = {}
